...
function buildPerspectiveCompositionPrompt(perspective: PerspectiveOption): string {
  if (perspective === "close") {
    return "近景只展示原始产品中一个明确存在的局部，不展示完整柜体。近景沿用远景与侧面的正常摆放逻辑：柜体仍然正常靠墙落地，背板与墙面平行，柜体自身保持垂直且方向不变；只把真实相机向柜体靠近并对准所选局部，近景效果只由相机靠近和取景裁切产生，禁止旋转、斜摆、拉伸或重构柜体。画面展示柜体整体约 15% 到 25% 的一个连续局部，并至少保留一个完整且可识别的结构单元，例如一个完整抽屉及相邻柜门边缘、一扇完整柜门及相邻开放格，或一段完整层板与立板连接；不得只生成孤立木纹、单个拉手或无法识别所属产品的微距材质图。近景不是产品重设计或三维结构补全，只能选择原始产品图中清晰可见、可直接核对的连续区域，再对该真实区域做近距离取景和环境光匹配。必须以原始产品图为唯一几何与纹理依据，原图中不清晰或不可见的侧面、内部结构、五金、纹理、雕花、层板、抽屉或模块一律不得推测、补画或替换；若某个区域不够清晰，改选原图中更清晰的结构单元。柜体其余部分可自然超出画面边缘，环境只保留少量墙面、地面或光影，禁止把完整柜体缩小后继续放在画面中。";
  }
...
function placementPlanForPerspective(analysis: SceneAnalysis, perspective: PerspectiveOption) {
  if (perspective === "close") {
    return {
      ...
      placement: "近景保持已确认的正常靠墙落地位置；柜体背板与墙面平行并完整贴墙，柜体本身不移动、不旋转、不倾斜。",
      facing: "柜体朝向保持不变；只把相机向柜体靠近并对准原始产品图中一个清晰可见的完整结构单元，展示整体约 15% 到 25%。",
      rationale: "近景效果只由相机靠近和取景裁切产生，产品仍按远景与侧面的正常方式摆放。",
...
export function buildQualityPrompt(
  ...
  return [
    ...
    perspective === "close"
      ? "近景质检不得要求画面出现完整柜体，也不得因为看不到完整模块数量而判定失败。近景应展示柜体整体约 15% 到 25% 的连续局部，至少包含一个完整且可识别的真实结构单元，并允许其余产品超出画面边缘；柜体仍须正常靠墙落地且方向不变，近景只由相机靠近和取景裁切产生。核对画面中可见的柜门、抽屉、开放格、层板、木纹、拉手、玻璃或连接关系是否确实来自目标产品；若虚构细节、生成相似款、柜体斜放、只剩孤立材质微距或仍以完整柜体为主体，必须判定 passed=false。"
      : perspective === "medium"
