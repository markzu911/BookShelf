...
function buildPerspectiveCompositionPrompt(perspective: PerspectiveOption, hasWideConsistencyReference = false): string {
  ...
  if (perspective === "medium") {
    return `侧面视角以柜体主体为画面核心。柜体背板与墙面保持平行并完整贴墙，柜体不得旋转或斜放。相机从远景机位明显靠近，并向左前方或右前方横移 25 到 40 度，形成与远景一眼可辨的侧向视差；侧板可见性只能来自相机视差。画面应清楚看到正面和一侧侧板、前沿与自然纵深关系。允许只展示柜体约 55% 到 75%，可自然裁掉一端、顶部或底部，不要求完整柜体；但所有可见模块、柜门、抽屉、开放格和层板必须与原始产品一致。禁止正面平视、仅放大远景、镜像翻转产品或通过旋转柜体制造侧面。${hasWideConsistencyReference ? "远景参考只用于锁定同一摆位与尺度，原始产品图仍是产品结构的最高优先级依据。" : ""}`;
  }
...
function placementPlanForPerspective(analysis: SceneAnalysis, perspective: PerspectiveOption) {
  ...
  return {
    ...
    facing: "柜体朝向保持不变且正面与墙面平行；只把相机向柜体左前方或右前方横移 25 到 40 度，通过相机视差清楚看到正面和一侧侧板。",
...
export function buildCameraVariationPrompt(
  ...
  const cameraInstruction = perspective === "close"
    ...
    : "侧面视角必须把相机从远景位置明显前移，并向左前方或右前方横移 25 到 40 度；允许只展示柜体约 55% 到 75%，可裁掉一端、顶部或底部。柜体自身保持背面贴墙且不旋转，侧板只能因相机视差而可见。";
  return [
    cabinetPlacementSystemPrompt,
    "这是同一摆放方案的换镜头任务。第一张输入图是刚刚生成并确认布局的远景主图；最后一张输入图是用户上传的原始柜体产品图。",
    "绝对禁止改变主图中的柜体位置、朝向、大小、款式、颜色、材质、模块结构、房间结构、家具、装饰、光影和人物。不得重新规划摆位，不得增加、删除或替换任何家具。",
    "原始产品图仍是产品结构的最高优先级依据。主图与新镜头中的目标柜体都必须是原始产品图中的同一件产品，不得生成相似款。",
    ...
    "柜体产品一致性为 P0：所有在新镜头中可见的模块数量与顺序、宽度关系、柜门、抽屉、开放格、层板、玻璃、柜脚、颜色、材质、木纹、拉手和连接细节必须与原始产品图及主图一致，不得增删、合并、拆分、重排或臆造。",
    cameraInstruction,
    perspective === "medium"
