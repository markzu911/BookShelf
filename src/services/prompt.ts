...
function buildPerspectiveCompositionPrompt(perspective: PerspectiveOption): string {
  if (perspective === "close") {
    return "近景只展示原始产品中一个明确存在的局部，不展示完整柜体。近景不是产品重设计或三维结构补全，只能选择原始产品图中清晰可见、可直接核对的连续区域，再对该真实区域做近距离裁取、放大和环境光匹配。相机必须贴近柜体，只展示柜体整体约 8% 到 18% 的真实局部，例如一只清晰可见的拉手与门板交界、一个抽屉边角、一段层板连接、玻璃门框或一小块清晰木纹；不能同时拼接多个细节。柜体应自然超出画面四周，环境只保留少量墙面、地面或光影。必须以原始产品图为唯一几何与纹理依据，原图中不清晰或不可见的侧面、内部结构、五金、纹理、雕花、层板、抽屉或模块一律不得推测、补画或替换；若某个细节不够清晰，改选原图中更清晰的区域。禁止把完整柜体缩小后继续放在画面中。";
  }
