...
const productIdentityPriorityPrompt = "指令优先级：未经 AI 改画的原始产品图定义产品事实，产品一致性为 P0，优先于用户备注、场景参考、摆放方案、软装方案、平台上下文、关键词和视角构图。任何低优先级要求与产品结构、数量、比例、颜色、材质或可见细节冲突时，必须忽略冲突部分。";

function buildPerspectiveCompositionPrompt(perspective: PerspectiveOption): string {
  if (perspective === "close") {
    return "近景只展示原始产品中一个明确存在的局部，不展示完整柜体。这是单次生成的近景源画面，不是最终裁切画面。近景沿用远景与侧面的正常摆放逻辑：柜体仍然正常靠墙落地，背板与墙面平行，柜体自身保持垂直且方向不变；禁止旋转、斜摆、拉伸或重构柜体。源画面展示柜体约 50% 到 70%，并把原始产品图中清晰可见的一个完整且可识别的结构单元放在画面中央安全取景区，例如一个完整抽屉及相邻柜门边缘、一扇完整柜门及相邻开放格，或一段完整层板与立板连接。最终近景由前端本地裁切中央安全取景区产生，AI 不得直接生成微距特写，不得只生成孤立木纹、单个拉手或无法识别所属产品的局部。近景不是产品重设计或三维结构补全，只能选择原始产品图中清晰可见、可直接核对的连续区域。必须以原始产品图为唯一几何与纹理依据，原图中不清晰或不可见的侧面、内部结构、五金、纹理、雕花、层板、抽屉或模块一律不得推测、补画或替换；若某个区域不够清晰，改选原图中更清晰的结构单元。";
  }
...
function placementPlanForPerspective(analysis: SceneAnalysis, perspective: PerspectiveOption) {
  if (perspective === "close") {
    return {
      ...
      placement: "近景源画面保持已确认的正常靠墙落地位置；柜体背板与墙面平行并完整贴墙，柜体本身不移动、不旋转、不倾斜。",
      facing: "柜体朝向保持不变；源画面展示柜体约 50% 到 70%，并把原始产品图中一个清晰可见的完整结构单元放在中央安全取景区。",
      rationale: "AI 只生成结构清晰的近景源画面，最终近景只由前端本地裁切产生，不再让 AI 重画微距局部。",
      avoid: [...analysis.placementPlan.avoid, "孤立木纹微距", "只拍单个拉手", "柜体斜放", "推测原图不可见结构"],
      candidates: []
    };
  }
...
export function buildAnalysisPrompt(extraContext = "", extraPrompt: string[] = [], userRequirements = ""): string {
  return [
    ...
    productIdentityPriorityPrompt,
    "把用户明确提出的摆放、保留、移除、软装、灯光和人物要求写入方案；这些要求只优先于 AI 默认建议，未明确时才由 AI 自行判断。",
    userRequirements ? `用户当前要求（不得改变原始产品）：${userRequirements}` : "用户尚未给出额外要求，请依据空间与产品自然规划。",
...
export function buildVirtualAnalysisPrompt(
  ...
  return [
    ...
    productIdentityPriorityPrompt,
    userRequirements ? `用户当前要求（不得改变原始产品）：${userRequirements}` : "",
    extraContext ? `平台传入上下文：${extraContext}` : "",
    extraPrompt.length ? `平台补充关键词：${extraPrompt.join("、")}` : ""
  ].filter(Boolean).join("\n");
}
...
export function buildGenerationPrompt(
  ...
  return [
    ...
    perspective === "close"
      ? "近景不执行全柜软装方案，不新增或改造产品细节；只有当所选真实局部本来就是开放格或台面时，才可保留原图中已经清晰可见的物品。"
      : `已确认软装方案：${JSON.stringify(analysis.stylingPlan)}`,
    perspective === "close"
      ? "禁止为了丰富画面而在近景产品区域新增书籍、饰品、植物、五金或装饰。"
      : "书籍与饰品需要真实落在开放格、层板、玻璃柜内部或斗柜台面上，遮挡关系正确；封闭柜门和抽屉表面不得凭空出现物品。",
    perspective === "close"
      ? "近景源画面只匹配原场景的光照和环境色，不套用固定木材或家具风格；产品颜色、材质和木纹只以原始产品图为准。"
      : "画面质感参考高端黑胡桃木家居摄影：暖色环境光、自然接地阴影、真实木纹、克制复古氛围、舒适生活感。不得把场景变成白底产品棚拍。",
    buildHumanModelPrompt(settings),
    perspective === "close"
      ? "近景源画面的中央安全取景区必须只保留产品结构；若用户勾选人物，人物只能出现在外围，不得占据或遮挡中央产品区域。"
      : "",
    settings.notes ? `用户额外要求（仅在不改变原始产品结构与外观的前提下逐项执行）：${settings.notes}` : "",
...
export function buildVirtualRoomPrompt(
  ...
  return [
    ...
    perspective === "close"
      ? "近景不执行全柜软装方案，不新增或改造产品细节；只使用原始产品图中清晰可见的真实局部。"
    ...
    perspective === "close"
      ? "近景源画面的中央安全取景区必须只保留产品结构；若用户勾选人物，人物只能出现在外围，不得占据或遮挡中央产品区域。"
      : "",
    settings.notes ? `用户额外要求（仅在不改变原始产品结构与外观的前提下执行）：${settings.notes}` : "",
...
export function buildQualityPrompt(
  ...
  return [
    ...
    perspective === "close"
      ? "当前待检图是近景源画面，不是最终本地裁切图。源画面应展示柜体约 50% 到 70%，柜体仍须正常靠墙落地且方向不变，并把至少一个完整且可识别的真实结构单元放在中央安全取景区。核对画面中可见的柜门、抽屉、开放格、层板、木纹、拉手、玻璃或连接关系是否确实来自目标产品；若虚构细节、生成相似款、柜体斜放、只剩孤立材质微距、中心区域被人物遮挡或仍以完整柜体远景为主体，必须判定 passed=false。最终近景由前端本地裁切，不得要求 AI 在源画面阶段直接生成 15% 到 25% 的微距局部。"
      : perspective === "medium"
    ...
    `已确认软装方案：${JSON.stringify(analysis.stylingPlan)}`,
    settings.notes ? `用户当前要求（仅在不改变原始产品结构与外观的前提下检查）：${settings.notes}` : "",
    extraContext ? `平台传入上下文：${extraContext}` : "",
    extraPrompt.length ? `平台补充关键词：${extraPrompt.join("；")}` : ""
  ].filter(Boolean).join("\n");
}
