...
assert.match(componentSource, /preserveOriginalProductImage\(file\)/);
assert.match(componentSource, /const \[furnitureSourceImage, setFurnitureSourceImage\] = useState<UploadedImage \| null>\(null\)/);
assert.match(componentSource, /const generationBatch = selectedPerspective === "close"[\s\S]*createPixelLockedCloseupScene\([\s\S]*: await generateScenes\(generationSettings\)/);
assert.doesNotMatch(componentSource, /createFixedCameraViews\(closeSource\.imageUrl/);
assert.match(imageSource, /export async function preserveOriginalProductImage/);
assert.match(imageSource, /export async function createPixelLockedCloseupScene/);
assert.match(imageSource, /removeConnectedStudioBackground\(pixels\.data, product\.width, product\.height\)/);
assert.match(imageSource, /return canvas\.toDataURL\("image\/png"\)/);
assert.match(imageSource, /const MAX_CLOSEUP_UPSCALE = 2/);
assert.match(imageSource, /imageSmoothingQuality = "high"/);
assert.match(imageSource, /const scale = Math\.min\(MAX_CLOSEUP_UPSCALE, desiredScale\)/);
assert.doesNotMatch(imageSource, /Math\.max\(outputWidth \* 0\.92 \/ cropWidth, outputHeight \* 0\.9 \/ cropHeight\)/);
assert.match(geminiSource, /if \(requestedPerspective === "close"\) throw new Error\("近景必须使用原始产品像素在前端本地合成"\)/);
