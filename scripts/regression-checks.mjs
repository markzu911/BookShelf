...
assert.doesNotMatch(componentSource, /furnitureSourceImage|preserveOriginalProductImage|createPixelLockedCloseupScene/);
assert.match(componentSource, /const sourceGenerationSettings: PlacementSettings = selectedPerspective === "close"[\s\S]*perspectives: \["medium"\][\s\S]*clarity: "2K"/);
assert.match(componentSource, /const generationBatch = await generateScenes\(sourceGenerationSettings\)/);
assert.match(componentSource, /const closeSource = images\.find\(\(image\) => image\.perspective === "medium"\) \|\| images\[0\]/);
assert.match(componentSource, /createGeneratedSceneCloseup\(closeSource\.imageUrl\)/);
assert.doesNotMatch(imageSource, /preserveOriginalProductImage|createPixelLockedCloseupScene|MAX_CLOSEUP_UPSCALE/);
assert.match(imageSource, /export async function createGeneratedSceneCloseup/);
assert.match(imageSource, /const cropScale = 0\.52/);
assert.match(imageSource, /imageSmoothingQuality = "high"/);
assert.match(geminiSource, /if \(requestedPerspective === "close"\) throw new Error\("近景必须通过一次侧面场景生成后在前端本地裁切"\)/);
